...
# Calculations:
lin_reg_weight_vector_y1 = np.matmul(np.linalg.pinv(np.matmul(a_final_matrix.T, a_final_matrix)), np.matmul(a_final_matrix.T, y1_vector))
lin_reg_weight_vector_y2 = np.matmul(np.linalg.pinv(np.matmul(a_final_matrix.T, a_final_matrix)), np.matmul(a_final_matrix.T, y2_vector))
...
print(f'STEP 1: {np.matmul(a_final_matrix.T, a_final_matrix)}')
print(f'STEP 2: {np.matmul(np.linalg.pinv(np.matmul(a_final_matrix.T, a_final_matrix)), a_final_matrix.T)}')
print(f'STEP 3 {np.matmul(np.matmul(np.linalg.pinv(np.matmul(a_final_matrix.T, a_final_matrix)), a_final_matrix.T), y1_vector)}')

print(calcMeanSqError(w0_y1, weights_y1, test_features, test_y1))
print(calcMeanSqError(w0_y2, weights_y2, test_features, test_y2))